...
class ECMP(EventMixin):
	...
	def get_routes(self,src, dst):
		...
		d=None
		for _ in graph_dict:
			if graph_dict[_]['isHost']:
				if graph_dict[_]['mac'] == src:
					s=_
				if graph_dict[_]['mac'] == dst:
					d=_
			else:
				if graph_dict[_]['dpid'] == src:
					s=_
				if graph_dict[_]['dpid'] == dst:
					d=_

		return shortest_pathes[s][d] 
		...
		if (packet.dst not in self.macTable) or (not isinstance(packet.next, ipv4)):
			...
			if src_switch == dst_switch:
				...
				if src_switch in self.graph.keys():
					...
					self.possible_routes = self.get_routes(src_switch, dst_switch)
